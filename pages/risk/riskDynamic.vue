	...
	<view>
		<view class="uni-common-pd">
			<view class="segmented-control" :class="styleType">
				<view class="segmented-control-item" :class="styleType"
				 v-for="(item, index) in titleList" :key="index" @click="onClickItem(index)">
					{{item}}
		...
		<view>
			<view v-show="current === 0">
				<scroll-view :style="'width: 100%;height: ' + scrollHeight + 'px;background-color: #FFFFFF;'"
				 :scroll-y="true">
					<view class="uni-list">
							...
							<view class="uni-list-navigate">
								<scroll-view class="scroll-view-body" scroll-x>
									<view class="scroll-view-item" hover-class="uni-list-cell-hover">
										<view class="scroll-view-item scroll-item-head">{{local.title}}</view>
			...
			<view v-show="current === 1">
				<scroll-view :style="'width: 100%;height:'+scrollHeight+'px;background-color: #FFFFFF;'" :scroll-y="true">
					<view class="uni-list">
						<view class="uni-list-cell" v-for="(local, keyL) in submitedList" :key="keyL">
							<view class="uni-list-cell-navigate-item list-commit-title">{{local.SiteName}}</view>
							<view class="list-commit-date">{{local.StepDate}}</view>
						</view>
					</view>
				</scroll-view>
			</view>
