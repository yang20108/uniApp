	...
	<view class="view-auto-height">
		...
		<search-picker v-if="siteDataList.length" ref="searchPicker" :mode="pickerLevel" :pickerList="siteDataList"
		 ...
		 <scroll-view class="view-auto-scroll" :scroll-y="true">
		 	<view class="uni-list">
		 		<block v-for="(dataI, dataK) in WarnDataList" :key="dataK">
		 			<early-list-item class="uni-list-cell" :iconColor="dataI.iconColor" :title="dataI.SiteName" :content="dataI.WarnUnitName"
		 			 :date="dataI.CreateDate" :username="dataI.CreateUserName" @onclick="clickListItem"></early-list-item>
		 		</block>
		 	</view>
		 </scroll-view>
	</view>
	...
	export default {
		data() {
			return {
				...
				WarnDataList: modelData.modelEarlyList()
			}
		},
		methods: {
			showPicker(e) {
