	...
	<view class="wrap">
		<view class="div-table">
			...
			<view v-bind:class="classBody" v-for="(row,index) in datas" :key="index">
				<view v-for="(cell,index1) in row" :key="index1" v-bind:class="cell.className==undefined?classCell:cell.className">
					<view v-if="row.length-1!=index1">{{cell.value}}</view>
					<view v-else class="warning-sign-state" :style="{backgroundColor: cell.value}">预警</view>
				</view>
			</view>
		</view>
	</view>
