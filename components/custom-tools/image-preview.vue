<template>
	<view v-if="show"
	 ...
	 @touchmove.stop.prevent>
					...
					<movable-view
					 ...
					 @change="movableChange">
						<image
						 ...
						 :src="img"
						 :style="{
							 height: 'row' === direction ? '300px' : '400px',
		...
		<view v-if="imgs.length > 0"
		 :style="{
			bottom: 'row' === direction ? '50%' : '10px',
			right: 'row' === direction ? '45%' : '0',
			transform: 'row' === direction ? 'rotateZ(90deg)' : 'rotateZ(0deg)'
		 }"
		 class="page">
			<text class="text">{{ index + 1 }} / {{ imgs.length }}</text>
		</view>
		<view v-if="false" class="save" @click.stop.prevent="save"><text class="text">保存</text></view>
		<view v-if="false" class="rotate" @click.stop.prevent="rotate"><text class="text">旋转</text></view>
		<view v-if="descs.length > 0 && descs.length == imgs.length && descs[index].length > 0"
		 :style="{
			top: 'row' === direction ? '50%' : '0',
			left: 'row' === direction ? '30%' : '0',
			width: 'row' === direction ? '130%' : '100%',
			transform: 'row' === direction ? 'rotateZ(90deg)' : 'rotateZ(0deg)'
		 }"
		 class="desc">{{ descs[index] }}</view>
	</view>
</template>
...
<script>
	 ...
	 * imgs[Array] 图片列表
	 * descs[Array] 描述列表
	 * opacity[Number] 透明度,0到1之间
	 * circular[Boolean] 循环预览
	 * direction[String] 预览方向 row横屏预览 vertical竖屏预览（默认）
	 */
	export default {
		name: 'image-preview',
		props: {
